...
def visual_interpolar_for_point(data, point_x):
    ...
    point = interp.interpolar_for_point(data, point_x)
...
def visual_piece_interpolar(data, input_x):
    ...
    xy = interp.piece_interpolar(data, input_x)
    for item in xy:
        x += [item[0]]
        y += [item[1]]
...
def visual_lagrange(data, point_x):
    ...
    xy = interp.lagrange_polimom(data, point_x)
    point_y = interp.lagrange_polimom(data, point_x)[1]
    ...
    y = [interp.lagrange_polimom(data, item)[1] for item in x]
